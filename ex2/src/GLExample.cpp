...
namespace cgCourse
{
	...
	bool GLExample::init()
	{
		...
		cube->setScaling(glm::vec3(.5, 0.5, 0.5));
        ...
        torus->setPosition(glm::vec3(0, -0.5, -1));
        torus->setScaling(glm::vec3(1.0, 1.0, 1.0));
        torus->setRotation(glm::radians(120.0f), glm::vec3(1.0f, 0.0f, 1.0f));
	...
	bool GLExample::update()
	{
        ...
        float angle = glfwGetTime() *0.001f;
        cube->setRotation(angle, glm::vec3(0.0f, 1.0f, 1.0f));
        torus->setRotation(angle, glm::vec3(0.0f, 1.0f, 0.0f));
		return true;
	}
	...
	void GLExample::renderTorus()
	{
        ...
        glm::mat3 normalMatrix = glm::transpose(glm::inverse((torus->getModelMatrix())));
